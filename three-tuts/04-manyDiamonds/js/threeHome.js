...
let scene, camera, renderer, stats, gui, octahedron, torus;
let octahedronArray = [];
...
function init() {
    ...
    // create a single octahedron
    // octahedron = new THREE.Mesh(geometryOcta, materialOcta);
    // scene.add(octahedron);
    // octahedron.position.y = -6;


    // create many in an array

    for (let i = 0; i < 15; i++){
        octahedronArray.push(new THREE.Mesh(geometryOcta, materialOcta))
        scene.add(octahedronArray[i]);
        octahedronArray[i].position.y = -2-i + Math.random();
        octahedronArray[i].position.x = Math.random() * 2;
        console.log(Math.random())
    }

    console.log(octahedronArray)
